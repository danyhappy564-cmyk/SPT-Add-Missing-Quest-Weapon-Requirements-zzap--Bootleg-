...
**Original Author:** theGuiltyMan (Alptug Albayrak) and contributors
**Original Repository:** SPT-Add-Missing-Quest-Weapon-Requirements
**Original Link:** https://github.com/theGuiltyMan/SPT-Add-Missing-Quest-Weapon-Requirements
**License:** MIT
...
**로드 순서를 뒤로 옮겼습니다** — `TraderRegistration + 9999`(309,999) → `PostLoad + 50000`(1,050,000).

이 모드는 DB에 이미 있는 퀘스트 객체의 무기 조건 배열을 **제자리에서 고칩니다.** 그래서 이 모드
뒤에 퀘스트를 쓰는 모드가 있으면 작업이 조용히 날아갑니다 — 특히 퀘스트 **객체를 통째로 교체**하는
모드면 확장된 배열이 버려지는 객체에 들어있어서 **완전히** 날아갑니다.

가상의 이야기가 아닙니다. [SptQuestLive](https://github.com/Viper-9/SptQeustLive) 가
`PostLoad + 1` 에서 이렇게 합니다:

```csharp
quests[questId] = quest;   // JSON 에서 새로 만든 객체로 통째 교체
```

예전 우선순위로는 이 모드가 **69만 단위 먼저** 돌아서, SptQuestLive 가 덮어쓰는 퀘스트는 전부
확장을 잃었습니다. 양쪽 다 아무 로그도 안 남기고요.

늦게 도는 건 비용이 없습니다. 퀘스트 조건은 요청마다 서빙되니 마감이 없고, **아이템 추가**와 달리
(그건 SPT `DatabaseIntegrityService` 때문에 `Preload` 강제입니다) 기존 퀘스트 수정에는 컷오프가
없습니다. 오히려 늦을수록 이 모드 목적에 유리합니다 — 그때쯤이면 모드 무기도 모드 퀘스트도 더
많이 들어와 있으니까요.

**검증**: 원작 테스트 스위트 + 로드 순서 회귀 테스트 6개 = **570개 전부 통과**합니다.
인게임 테스트는 아직 안 했습니다.
